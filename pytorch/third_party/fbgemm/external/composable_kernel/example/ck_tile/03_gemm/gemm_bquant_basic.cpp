    ...
    const auto Run = [&](const auto has_hot_loop_, const auto tail_number_) {
        ...
        using GemmEpilogue        = ck_tile::CShuffleEpilogue<
                   ck_tile::CShuffleEpilogueProblem<ADataType,
                                                    ...
                                                    K_Warp_Tile,
                                                    transposed_warp_gemm>>;
        using Kernel = ck_tile::BQuantGemmKernel<TilePartitioner, CodegenGemmPipeline, GemmEpilogue>;
...
int run_gemm_example_prec_type(std::string a_layout, std::string b_layout, int argc, char* argv[])
{
                 ...
                 std::is_same_v<typename TypeConfig::BDataType, ck_tile::bf8_t>)
    {
        ...
        {
            return run_gemm_example_with_layouts<TypeConfig,
                                                 QuantGroupSize>(
...
int run_gemm_example(int argc, char* argv[])
{
    ...
    if(data_type == "fp8")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::half_t>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else if(data_type == "bf8")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::bf8_t, ck_tile::bf8_t, ck_tile::half_t>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else if(data_type == "fp8i4")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::fp8_t, ck_tile::pk_int4_t, float, ck_tile::fp8_t>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else if(data_type == "bf8i4")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::bf8_t, ck_tile::pk_int4_t, float, ck_tile::bf8_t>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else if(data_type == "fp8i4f32")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::fp8_t, ck_tile::pk_int4_t, float, float>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else if(data_type == "bf8i4f32")
    {
        using TypeConfig = decltype(GemmQuantTypeConfig<ck_tile::bf8_t, ck_tile::pk_int4_t, float, float>{});
        return run_gemm_example_prec_type<TypeConfig, 128>(
            a_layout, b_layout, argc, argv);
    }
    else
