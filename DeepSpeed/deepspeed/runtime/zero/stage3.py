...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        (self.total_model_params,
         self.largest_layer_params,
         self.largest_layer_param_bytes,
         self.largest_layer_name) = model_to_layer_param_stats(module)
        ...
        self._ipc_undo_log_total_seconds = 0.0
        self._ipc_undo_log_step_seconds = 0.0
        self._ipc_undo_log_last_step_seconds = 0.0
        self._ipc_undo_log_step_samples = 0
    ...
    def _setup_for_real_optimizer(self):
        ...
        total_grad_elements = sum(p.partition_numel() for p in all_params)

        _ipc_tool, _ipc_group = self._zero3_ipc_tool()
        if _ipc_tool is not None:
            with _ipc_tool.scope(_ipc_group, "grad_partitions_flat_buffer"):
                self.grad_partitions_flat_buffer = torch.zeros(total_grad_elements,
                                                               device=self.device,
    ...
    def _zero3_ipc_group_name(self):
        base_group = os.environ.get("DEEPSPEED_ZERO3_IPC_GROUP", "").strip()
        if not base_group:
            return None
        return f"{base_group}__optimizer_z3"

    def _zero3_ipc_tool(self):
        group_name = self._zero3_ipc_group_name()
        if group_name is None:
            return None, None

        if self.offload_optimizer or self.swap_optimizer or self.params_in_nvme_and_cpu:
            return None, group_name

        module_name = os.environ.get("DEEPSPEED_ZERO3_IPC_TOOL_MODULE", "ds_tool").strip() or "ds_tool"
        try:
            module = __import__(module_name)
        except Exception as exc:
            raise RuntimeError(f"Failed to import ZeRO-3 IPC tool module '{module_name}': {exc}") from exc

        getter = getattr(module, "get_active_tool", None)
        tool = getattr(module, "TOOL", None) if getter is None else getter()
        if tool is None:
            raise RuntimeError(
                f"ZeRO-3 IPC tool module '{module_name}' is loaded but no active interceptor is available."
            )
        return tool, group_name

    def _zero3_ipc_group_exists(self, tool, group_name):
        cached = getattr(self, "_ipc_zero3_optimizer_group_exists", None)
        if cached is None:
            cached = tool.get_size(group_name) > 0
            self._ipc_zero3_optimizer_group_exists = cached
            if not cached and os.environ.get("DEEPSPEED_ZERO3_IPC_REQUIRE_EXISTING", "0") == "1":
                raise RuntimeError(f"IPC ZeRO-3 optimizer group '{group_name}' does not exist.")
        return cached

    def _zero3_ipc_subgroup_tensor_names(self, sub_group_id):
        tensor_names = [
            f"fp32_subgroups[{sub_group_id}]",
            f"fp32_subgroups[{sub_group_id}].exp_avg",
            f"fp32_subgroups[{sub_group_id}].exp_avg_sq",
        ]
        param_group_id = self.sub_group_to_group_id[sub_group_id]
        if self.optimizer.param_groups[param_group_id].get("amsgrad", False):
            tensor_names.append(f"fp32_subgroups[{sub_group_id}].max_exp_avg_sq")
        return tensor_names

    def _zero3_ipc_get_optimizer_step(self, sub_group_id):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return None
        return tool.zero3_get_optimizer_step(group_name, sub_group_id)

    def _zero3_ipc_set_optimizer_step(self, sub_group_id, step_value):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        tool.zero3_set_optimizer_step(group_name, sub_group_id, step_value)

    def _zero3_ipc_begin_step(self):
        self._ipc_undo_log_step_seconds = 0.0
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        tool.zero3_begin_step(group_name, len(self.fp16_groups), self.loss_scale)

    def _zero3_ipc_begin_subgroup_update(self, sub_group_id):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        fp32_param = self.fp32_partitioned_groups_flat[sub_group_id]
        step_value = self.optimizer.state.setdefault(fp32_param, {}).get("step", 0)
        undo_log_start = time.perf_counter()
        tool.zero3_begin_subgroup(group_name,
                                  sub_group_id,
                                  self._zero3_ipc_subgroup_tensor_names(sub_group_id),
                                  step_value)
        undo_log_seconds = time.perf_counter() - undo_log_start
        self._ipc_undo_log_step_seconds += undo_log_seconds
        self._ipc_undo_log_total_seconds += undo_log_seconds

    def _zero3_ipc_commit_subgroup_update(self, sub_group_id, next_sub_group):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        fp32_param = self.fp32_partitioned_groups_flat[sub_group_id]
        step_value = self.optimizer.state.setdefault(fp32_param, {}).get("step", 0)
        self._zero3_ipc_set_optimizer_step(sub_group_id, step_value)
        tool.zero3_commit_subgroup(group_name, sub_group_id, next_sub_group, step_value)

    def _zero3_ipc_finish_step(self):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        tool.zero3_finish_step(group_name)
        self._ipc_undo_log_last_step_seconds = float(self._ipc_undo_log_step_seconds)
        self._ipc_undo_log_step_samples += 1

    def _zero3_ipc_get_step_journal(self):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return None
        return tool.zero3_get_step_journal(group_name)

    def _zero3_ipc_rollback_active_subgroup(self):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return None
        return tool.zero3_rollback_active_subgroup(group_name)

    def _zero3_ipc_restore_averaged_gradients_from_buffer(self):
        if self.offload_optimizer:
            return
    ...
    def _zero3_ipc_resume_log(self, message):
        if dist.get_rank() == 0:
            print(message, flush=True)
            logger.info(message)

    def _zero3_ipc_maybe_inject_fault(self, sub_group_id, stage):
        fault_sub_group = os.environ.get("DEEPSPEED_ZERO3_IPC_FAULT_SUBGROUP")
        fault_stage = os.environ.get("DEEPSPEED_ZERO3_IPC_FAULT_STAGE", "after_optimizer_step")
        if fault_sub_group is None:
            return
        try:
            target_sub_group = int(fault_sub_group)
        except (TypeError, ValueError):
            return
        if target_sub_group != int(sub_group_id) or fault_stage != stage:
            return
        exit_code = int(os.environ.get("DEEPSPEED_ZERO3_IPC_FAULT_EXIT_CODE", "86"))
        logger.warning(
            "Injecting configured ZeRO-3 IPC fault at subgroup=%s stage=%s with exit_code=%s",
            sub_group_id,
            stage,
            exit_code,
        )
    ...
    def resume_ipc_inflight_step_if_needed(self):
        self._ipc_resume_state = {"resumed": False}
        journal = self._zero3_ipc_get_step_journal()
        if not journal or not journal.get("inflight", False):
            return False

        resume_state = {
            "resumed": True,
            "detected_active_sub_group": journal.get("active_sub_group"),
            "resume_start_sub_group": journal.get("next_sub_group"),
            "resume_total_subgroups": journal.get("total_subgroups"),
            "rolled_back_sub_group": None,
            "replayed_sub_groups": [],
        }
        self._ipc_resume_state = resume_state

        self._zero3_ipc_resume_log(
            f"[IPC-Resume] detected in-flight ZeRO-3 step: "
            f"active_sub_group={journal.get('active_sub_group')} "
            f"next_sub_group={journal.get('next_sub_group')} "
            f"total_subgroups={journal.get('total_subgroups')}"
        )

        active_sub_group = journal.get("active_sub_group")
        if active_sub_group is not None:
            resume_state["rolled_back_sub_group"] = int(active_sub_group)
            self._zero3_ipc_resume_log(f"[IPC-Resume] rolling back active sub_group={active_sub_group}")
            self._zero3_ipc_rollback_active_subgroup()
            journal = self._zero3_ipc_get_step_journal() or journal
            self._zero3_ipc_resume_log(
                f"[IPC-Resume] rollback finished, next_sub_group={journal.get('next_sub_group')}"
            )

        start_sub_group = int(journal.get("next_sub_group", 0))
        resume_state["resume_start_sub_group"] = start_sub_group
        if journal.get("loss_scale", None) is not None:
            self.loss_scale = journal["loss_scale"]

        for sub_group_id in range(len(self.fp16_groups)):
            self._reassign_or_swap_out_partitioned_parameters(sub_group_id)

        if start_sub_group >= len(self.fp16_groups):
            self._zero3_ipc_resume_log("[IPC-Resume] no remaining subgroup updates; finishing pending step journal")
            self._zero3_ipc_finish_step()
            return True

        self._zero3_ipc_resume_log(
            f"[IPC-Resume] resuming optimizer step from sub_group={start_sub_group} "
            f"to sub_group={len(self.fp16_groups) - 1}"
        )
        self._pre_step()
        self._partition_all_parameters()
        self._zero3_ipc_restore_averaged_gradients_from_buffer()
        ...
        for sub_group_id in range(start_sub_group, len(self.fp16_groups)):
            resume_state["replayed_sub_groups"].append(int(sub_group_id))
            self._zero3_ipc_resume_log(f"[IPC-Resume] replaying sub_group={sub_group_id}")
            self._zero3_ipc_begin_subgroup_update(sub_group_id)
            self._zero3_ipc_maybe_inject_fault(sub_group_id, "after_begin")
            self._prepare_sub_group(sub_group_id, timer_names)
            self.unscale_and_clip_grads(sub_group_id, scaled_global_grad_norm)
            self._optimizer_step(sub_group_id)
            self._zero3_ipc_maybe_inject_fault(sub_group_id, "after_optimizer_step")
            self._reassign_or_swap_out_partitioned_parameters(sub_group_id)
            self._zero3_ipc_commit_subgroup_update(sub_group_id, sub_group_id + 1)
            self._zero3_ipc_maybe_inject_fault(sub_group_id, "after_commit")
            self._release_sub_group(sub_group_id, timer_names)
            self._zero3_ipc_resume_log(f"[IPC-Resume] committed recovered sub_group={sub_group_id}")

        self.timers(OPTIMIZER_STEP_TIMER).stop()
        self._post_step(timer_names)
        self._zero3_ipc_finish_step()
        self._zero3_ipc_resume_log("[IPC-Resume] finished replaying pending ZeRO-3 step")
        return True

    def _zero3_ipc_initialize_adam_state(self, sub_group_id):
        tool, group_name = self._zero3_ipc_tool()
        if tool is None:
            return
        ...
        num_elements = fp32_param.numel()
        target_device = fp32_param.device
        if "exp_avg" not in state:
            with tool.scope(group_name, f"fp32_subgroups[{sub_group_id}].exp_avg"):
                state["exp_avg"] = torch.zeros(num_elements, device=target_device, dtype=torch.float)
        if "exp_avg_sq" not in state:
            with tool.scope(group_name, f"fp32_subgroups[{sub_group_id}].exp_avg_sq"):
                state["exp_avg_sq"] = torch.zeros(num_elements, device=target_device, dtype=torch.float)

        param_group_id = self.sub_group_to_group_id[sub_group_id]
        if self.optimizer.param_groups[param_group_id].get("amsgrad", False):
            if "max_exp_avg_sq" not in state:
                with tool.scope(group_name, f"fp32_subgroups[{sub_group_id}].max_exp_avg_sq"):
                    state["max_exp_avg_sq"] = torch.zeros(num_elements, device=target_device, dtype=torch.float)

        if "step" not in state:
            persisted_step = self._zero3_ipc_get_optimizer_step(sub_group_id)
            state["step"] = 0 if persisted_step is None else persisted_step
            if persisted_step is None:
                self._zero3_ipc_set_optimizer_step(sub_group_id, state["step"])
        ...
        for i, tensor in enumerate(self.fp16_partitioned_groups_flat):
            ...
            if self._swappable_optimizer_subgroup(i):
                ...
                if self.params_in_nvme_and_cpu and tensor is None:
                    ...
                    _ipc_tool, _ipc_group = self._zero3_ipc_tool()
                    if _ipc_tool is not None:
                        target_device = self.subgroup_to_device[i] if self.offload_optimizer else self.device
                        src_partition = self.fp16_partitioned_groups_flat[i]
                        group_existed = self._zero3_ipc_group_exists(_ipc_tool, _ipc_group)
                        with _ipc_tool.scope(_ipc_group, f"fp32_subgroups[{i}]"):
                            ipc_fp32_partition = torch.empty(num_elements, device=target_device, dtype=torch.float)
                        if not group_existed:
                            with torch.no_grad():
                                ipc_fp32_partition.copy_(
                                    src_partition.to(device=ipc_fp32_partition.device, dtype=torch.float))
                        self.fp32_partitioned_groups_flat.append(ipc_fp32_partition)
    ...
    def initialize_optimizer_states(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            self._zero3_ipc_initialize_adam_state(i)
...
def model_to_layer_param_stats(model):
    total_params = 0
    total_seen = set()
    largest_layer_name = ""
    ...
    for module_name, module in model.named_modules():
        ...
        for param in module.parameters(recurse=False):
            ...
            if storage_key not in total_seen:
                total_seen.add(storage_key)
    ...
    return total_params, largest_layer_params, largest_layer_param_bytes, largest_layer_name


def model_to_params(model):
    total_params, largest_layer_params, _, _ = model_to_layer_param_stats(model)
