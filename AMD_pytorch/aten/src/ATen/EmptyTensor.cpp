...
namespace at::detail {
  ...
  struct IpcDeleterContext {
    ...
    ~IpcDeleterContext() {
      if (mapped_ptr) {
        gpuIpcCloseMemHandle(mapped_ptr);
      }
      try {
  ...
  static TensorBase ipc_alloc_tensor_via_daemon(
    ...
    int fd = ipc_socket_connect(g_ipc_socket_path);
    ...
    std::memcpy(&ipc_handle, p, sizeof(gpuIpcMemHandle_t));
    p += sizeof(gpuIpcMemHandle_t);
  
    uint64_t resp_alloc_id = decode_u64(p);
    uint64_t resp_size_bytes = decode_u64(p);
    (void)resp_alloc_id;
    void* mapped_ptr = nullptr;
    gpuError_t err = gpuIpcOpenMemHandle(&mapped_ptr, ipc_handle, gpuIpcMemLazyEnablePeerAccess);
    TORCH_CHECK(err == gpuSuccess, "IPC: gpuIpcOpenMemHandle failed: ", gpuGetErrorString(err));

    auto* ctx = new IpcDeleterContext{mapped_ptr, alloc_id, g_ipc_socket_path};
