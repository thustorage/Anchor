...
class DeviceMemoryProfiler:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        gc.collect()
        vllm_tool = _get_ipc_tool_state()
        if (
            vllm_tool is not None
            and vllm_tool.ipc_enabled()
            and vllm_tool.TOOL is not None
            and vllm_tool.disguise_switch
        ):
            self.consumed_memory += vllm_tool.TOOL.get_size("Weight")
...
class MemorySnapshot:
    ...
    def measure(self) -> None:
        ...
        if (
            ...
            self.free_memory += vllm_tool.TOOL.get_size("Weight")

        if (
            vllm_tool is not None
            and vllm_tool.ipc_enabled()
            and vllm_tool.TOOL is not None
            and vllm_tool.disguise_kv
        ):
            self.free_memory += vllm_tool.TOOL.get_size("KV")
        ...
        if (
            ...
            and vllm_tool.ipc_enabled()
            and vllm_tool.TOOL is not None
            and not vllm_tool.disguise_switch
        ):
            self.torch_peak += vllm_tool.TOOL.get_size("Weight")
            self.torch_memory += vllm_tool.TOOL.get_size("Weight")

        if (
            vllm_tool is not None
            and vllm_tool.ipc_enabled()
            and vllm_tool.TOOL is not None
            and not vllm_tool.disguise_kv
        ):
            self.torch_peak += vllm_tool.TOOL.get_size("KV")
            self.torch_memory += vllm_tool.TOOL.get_size("KV")
