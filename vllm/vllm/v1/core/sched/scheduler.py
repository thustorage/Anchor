...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.debug_prefill_tokens = os.environ.get(
            "VLLM_DEBUG_SCHEDULE_PREFILL_TOKENS", ""
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if self.debug_prefill_tokens and num_scheduled_tokens:
            ...
            for request in scheduled_requests:
                ...
                request_prefill_tokens = min(scheduled_tokens, prompt_tokens_remaining)
                prefill_tokens += request_prefill_tokens
                decode_tokens += scheduled_tokens - request_prefill_tokens
                if request_prefill_tokens > 0:
                    prefill_req_ids.append(req_id)
            if prefill_tokens > 0:
                logger.info(
                    "[schedule-debug] prefill_tokens=%d decode_tokens=%d "
                    "scheduled_reqs=%d prefill_reqs=%d prefill_req_ids=%s",
                    prefill_tokens,
                    decode_tokens,
