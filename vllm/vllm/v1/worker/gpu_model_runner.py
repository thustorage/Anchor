...
from vllm.v1.worker.kv_cpu_buffer import prepare_kv_cpu_buffer
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.kv_cache_tensors_by_layer: dict[str, Any] = {}
        self.kv_checkpoint_load_seconds: float = 0.0
    ...
    def _load_kv_cache_tensors_from_cpu_checkpoint(
        self, checkpoint_path: str, kv_cache_config: KVCacheConfig
    ) -> tuple[dict[str, Any], torch.Tensor | None, type[AttentionBackend] | None]:
        logger.info(
            "Loading KV cache tensors from CPU checkpoint: %s", checkpoint_path
        )
        payload = torch.load(
        ...
        if not isinstance(payload, dict):
            raise TypeError(
                "KV checkpoint payload must be a dict or a wrapper dict with "
                f"'kv_caches', got {type(payload)!r}."
            )

        if "kv_caches" in payload:
            kv_caches = payload["kv_caches"]
            cross_layers_kv_cache = payload.get("cross_layers_kv_cache")
        elif "cross_layers_kv_cache" in payload:
            cross_layers_kv_cache = payload["cross_layers_kv_cache"]
            cache_dtype = self.cache_config.cache_dtype
            if not self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
                raise ValueError(
                    "cross_layers_kv_cache checkpoint payload requires a "
                    "uniform KV cache layout."
                )
            kv_caches, _, attn_backend = self.allocate_uniform_kv_caches_from_tensor(
        ...
        if not isinstance(kv_caches, dict):
            raise TypeError(
                "KV checkpoint field 'kv_caches' must be a dict mapping layer "
                f"names to tensors, got {type(kv_caches)!r}."
            )
        if cross_layers_kv_cache is not None and not isinstance(
            cross_layers_kv_cache, torch.Tensor
        ):
            raise TypeError(
                "KV checkpoint field 'cross_layers_kv_cache' must be a tensor, "
                f"got {type(cross_layers_kv_cache)!r}."
            )
        ...
        missing_layers = expected_layers - set(kv_caches)
        if missing_layers:
            raise KeyError(
                "KV checkpoint is missing required layer tensors: "
                f"{sorted(missing_layers)!r}"
            )
        ...
        if cross_layers_kv_cache is not None:
            ...
            if not self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
                raise ValueError(
                    "cross_layers_kv_cache can only be restored when uniform "
                    "KV cache layout is enabled."
                )
    ...
    def _parse_torch_dtype(dtype_name: str) -> torch.dtype:
        if not isinstance(dtype_name, str):
            raise TypeError(f"dtype_name must be a string, got {type(dtype_name)!r}.")

        attr_name = dtype_name.split(".")[-1]
    ...
    def _expand_block_ids(
        block_ids: list[int], block_size_factor: int
    ) -> np.ndarray:
        if not block_ids:
    ...
    @classmethod
    def _copy_selected_blocks_from_cpu_buffer(
        cls,
        *,
        shared_buffer: memoryview,
        entry: dict[str, Any],
        target: torch.Tensor,
        block_ids: list[int],
    ) -> bool:
        ...
        block_size_factor = entry.get("block_size_factor")
        kv_dim_before_num_blocks = entry.get("kv_dim_before_num_blocks")
        if not isinstance(group_id, int):
            return False
        if not isinstance(block_size_factor, int) or block_size_factor <= 0:
            return False
        if not isinstance(kv_dim_before_num_blocks, bool):
            return False

        dtype = cls._parse_torch_dtype(entry["dtype"])
        ...
        if tuple(target.shape) != shape:
            raise ValueError(
                "KV tensor shape mismatch while restoring from shared CPU buffer: "
                f"target={tuple(target.shape)!r} source={shape!r}"
            )
        ...
        expanded_ids = cls._expand_block_ids(block_ids, block_size_factor)
        if expanded_ids.size == 0:
            del source
            return True

        if kv_dim_before_num_blocks:
            num_kernel_blocks = int(source.shape[1])
        else:
            num_kernel_blocks = int(source.shape[0])
        max_block_id = int(expanded_ids.max())
        if max_block_id >= num_kernel_blocks:
            raise ValueError(
                "KV CPU buffer references block id outside tensor bounds: "
                f"max_block_id={max_block_id} num_kernel_blocks={num_kernel_blocks} "
                f"group_id={group_id}"
            )
        ...
        if kv_dim_before_num_blocks:
            ops.swap_blocks(source[0], target[0], block_pairs_tensor)
            ops.swap_blocks(source[1], target[1], block_pairs_tensor)
        else:
            ops.swap_blocks(source, target, block_pairs_tensor)

        del source
        return True

    @classmethod
    def _copy_tensor_from_cpu_buffer(
        cls,
        *,
        shared_buffer: memoryview,
        entry: dict[str, Any],
    ...
    def _restore_kv_cache_tensors_from_cpu_buffer(
        self,
        buffer_metadata_json: str,
        kv_caches: dict[str, torch.Tensor],
        cross_layers_kv_cache: torch.Tensor | None,
    ) -> None:
        logger.info("Loading KV cache tensors from shared CPU buffer metadata")
        buffer_metadata = json.loads(buffer_metadata_json)
        if not isinstance(buffer_metadata, dict):
            raise TypeError(
                "KV CPU buffer metadata must decode to a dict, "
                f"got {type(buffer_metadata)!r}."
            )

        if buffer_metadata.get("format") != "vllm_kv_cpu_buffer_v1":
            raise ValueError(
                "Unsupported KV CPU buffer metadata format: "
                f"{buffer_metadata.get('format')!r}"
            )

        buffer_name = buffer_metadata.get("buffer_name")
        if not isinstance(buffer_name, str) or not buffer_name:
            raise ValueError("KV CPU buffer metadata is missing buffer_name.")

        entries = buffer_metadata.get("entries")
        if not isinstance(entries, list):
            raise TypeError(
                "KV CPU buffer metadata field 'entries' must be a list, "
                f"got {type(entries)!r}."
            )

        valid_block_ids = buffer_metadata.get("valid_block_ids")
        if valid_block_ids is not None and not isinstance(valid_block_ids, list):
            raise TypeError(
                "KV CPU buffer metadata field 'valid_block_ids' must be a list, "
                f"got {type(valid_block_ids)!r}."
            )

        shm, _ = prepare_kv_cpu_buffer(buffer_name, pin_memory=self.pin_memory)
        buffer_kind = buffer_metadata.get("kind")
        if buffer_kind == "cross_layers_kv_cache":
            if cross_layers_kv_cache is None:
                raise ValueError(
                    "Received cross-layer KV CPU buffer metadata, but "
                    "cross_layers_kv_cache is not allocated."
                )
            if len(entries) != 1:
                raise ValueError(
                    "cross_layers_kv_cache metadata must contain exactly one "
                    f"entry, got {len(entries)}."
                )
            copied = False
            if isinstance(valid_block_ids, list) and valid_block_ids:
                block_ids = [int(block_id) for block_id in valid_block_ids[0]]
                copied = self._copy_selected_blocks_from_cpu_buffer(
                    shared_buffer=shm.buf,
                    entry=entries[0],
                    target=cross_layers_kv_cache,
                    block_ids=block_ids,
                )
            if not copied:
                self._copy_tensor_from_cpu_buffer(
                    shared_buffer=shm.buf,
                    entry=entries[0],
                    target=cross_layers_kv_cache,
                )
        elif buffer_kind == "kv_caches":
            for entry in entries:
                tensor_name = entry.get("name")
                if not isinstance(tensor_name, str):
                    raise TypeError(
                        "KV CPU buffer entry field 'name' must be a string, "
                        f"got {type(tensor_name)!r}."
                    )
                if tensor_name not in kv_caches:
                    raise KeyError(
                        "KV CPU buffer references an unknown layer tensor: "
                        f"{tensor_name!r}"
                    )
                copied = False
                if isinstance(valid_block_ids, list):
                    group_id = entry.get("kv_cache_group_id")
                    if isinstance(group_id, int) and 0 <= group_id < len(valid_block_ids):
                        block_ids = [int(block_id) for block_id in valid_block_ids[group_id]]
                        copied = self._copy_selected_blocks_from_cpu_buffer(
                            shared_buffer=shm.buf,
                            entry=entry,
                            target=kv_caches[tensor_name],
                            block_ids=block_ids,
                        )
                if not copied:
                    self._copy_tensor_from_cpu_buffer(
            ...
            raise ValueError(
                "Unsupported KV CPU buffer kind: "
                f"{buffer_kind!r}"
            )

    @staticmethod
    def allocate_uniform_kv_caches_from_tensor(
        cross_layers_kv_cache: torch.Tensor,
        kv_cache_config: KVCacheConfig,
    ...
    def _allocate_fresh_kv_cache_tensors(
        self, kv_cache_config: KVCacheConfig, kernel_block_sizes: list[int]
    ) -> tuple[dict[str, torch.Tensor], torch.Tensor | None, type[AttentionBackend] | None]:
        vllm_tool = None
        import os
    ...
    def initialize_kv_cache_tensors(
        ...
        self.kv_checkpoint_load_seconds = 0.0

        kv_buffer_metadata = envs.VLLM_KV_CPU_BUFFER_METADATA
        kv_checkpoint_path = envs.VLLM_KV_CPU_CHECKPOINT_PATH
        if kv_buffer_metadata:
            kv_checkpoint_load_started_at = time.time()
            kv_caches, cross_layers_kv_cache, attn_backend = (
                self._allocate_fresh_kv_cache_tensors(
                    kv_cache_config, kernel_block_sizes
                )
            )
            self._restore_kv_cache_tensors_from_cpu_buffer(
                kv_buffer_metadata, kv_caches, cross_layers_kv_cache
            )
            self.kv_checkpoint_load_seconds = (
                time.time() - kv_checkpoint_load_started_at
            )
            self.cross_layers_kv_cache = cross_layers_kv_cache
            self.cross_layers_attn_backend = attn_backend
        elif kv_checkpoint_path:
            kv_checkpoint_load_started_at = time.time()
            kv_caches, cross_layers_kv_cache, attn_backend = (
                self._load_kv_cache_tensors_from_cpu_checkpoint(
                    kv_checkpoint_path, kv_cache_config
                )
            )
            self.kv_checkpoint_load_seconds = (
                time.time() - kv_checkpoint_load_started_at
        ...
        self.kv_cache_tensors_by_layer = kv_caches
