...
class Worker(WorkerBase):
    ...
    def determine_available_memory(self) -> int:
        ...
        if os.environ.get("VLLM_IPC_TOOL", "").lower() in {"1", "true", "yes", "on"}:
            import importlib
            import sys as _sys

            _ipc_tools_path = os.environ.get(
                "VLLM_IPC_TOOLS_PATH",
                next((str(_p / "IPC" / "ipc_tools")
                      for _p in __import__("pathlib").Path(__file__).resolve().parents
                      if (_p / "IPC" / "ipc_tools").is_dir()), ""),
            )
            if _ipc_tools_path not in _sys.path:
                _sys.path.insert(0, _ipc_tools_path)
            _vllm_tool = importlib.import_module(
                os.environ.get("VLLM_IPC_TOOL_MODULE", "vllm_tool")
            )
            if _vllm_tool.ipc_enabled() and _vllm_tool.TOOL is not None:
                self.available_kv_cache_memory_bytes = (
                    _vllm_tool.TOOL.process_vllm_kv_memory(
