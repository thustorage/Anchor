...
class WorkerProc:
    ...
    def __init__(
        ...
        if vllm_tool is not None and vllm_tool.ipc_enabled():
            ...
            if hasattr(vllm_tool, "initialize_tool"):
                vllm_tool.initialize_tool(load_device=load_device, local_rank=local_rank)
            else:
                import tool

                target_device = torch.device(load_device)
                if target_device.type == "cuda" and target_device.index is None:
                    target_device = torch.device(target_device.type, local_rank)
                if target_device.type == "cuda":
                    torch.cuda.set_device(target_device)
                vllm_tool.TOOL = tool.TensorFactoryInterceptor(target_device=target_device)
        ...
        if vllm_tool is not None and vllm_tool.ipc_enabled():
            vllm_tool.disguise_switch = False
