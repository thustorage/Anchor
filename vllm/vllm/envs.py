...
if TYPE_CHECKING:
    ...
    VLLM_KV_CPU_CHECKPOINT_PATH: str | None = None
    VLLM_KV_CPU_BUFFER_METADATA: str | None = None
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Optional path to a CPU-side KV checkpoint payload. When set, KV cache
    # initialization loads tensors from this checkpoint instead of allocating
    # fresh storage on the GPU.
    "VLLM_KV_CPU_CHECKPOINT_PATH": lambda: os.environ.get(
        "VLLM_KV_CPU_CHECKPOINT_PATH", None
    ),
    # Optional JSON metadata describing a shared CPU buffer that contains raw
    # KV cache tensor payloads. When set, KV cache initialization allocates the
    # normal GPU tensors and copies values from the shared CPU buffer.
    "VLLM_KV_CPU_BUFFER_METADATA": lambda: os.environ.get(
        "VLLM_KV_CPU_BUFFER_METADATA", None
    ),
