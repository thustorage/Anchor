...
# =====================================================================
# Unified DS-native checkpoint baseline runner.
# The bench derives its launch mode from the --gpu-ids count: one id ->
# single-GPU (direct launch, exit-86 fault), several -> torchrun multi-GPU.
#
# Usage:
#   ./run_ds_hf_three_models.sh [single|multi]
#
...
#   GPU_IDS=0,1,2,3 ./run_ds_hf_three_models.sh multi
# =====================================================================
...
case "${MODE}" in
  ...
  multi)
    GPU_IDS="${GPU_IDS:-0,1}"
    BASELINE_NAME="${BASELINE_NAME:-ds_native_multigpu}"
...
# Remove run artifacts on exit (checkpoints); reports under REPORT_DIR are kept.
cleanup() {
...
run_model() {
  ...
  echo "[run] label=${label} model=${model_path} gpu_ids=${GPU_IDS} batch_size=${MODEL_BATCH_SIZE} seq_len=${MODEL_SEQ_LEN} steps_per_epoch=${MODEL_STEPS_PER_EPOCH} log_interval=${MODEL_LOG_INTERVAL}"
