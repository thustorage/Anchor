...
# =====================================================================
# Unified CheckFreq baseline runner (ZeRO-3). The bench launches
# torch.distributed.run itself (nproc = number of --gpu-ids), so single-GPU
# is just GPU_IDS with one id (world_size 1).
#
# Usage:
#   ./run_checkfreq_hf_three_models.sh [single|multi]
#
...
#   GPU_IDS=0,1,2,3 ./run_checkfreq_hf_three_models.sh multi
# =====================================================================
...
case "${MODE}" in
  single)
    GPU_IDS="${GPU_IDS:-0}"
    BASELINE_NAME="${BASELINE_NAME:-checkfreq}"
    MASTER_PORT_BASE="${MASTER_PORT_BASE:-29731}"
    ;;
  multi)
    GPU_IDS="${GPU_IDS:-0,1}"
    BASELINE_NAME="${BASELINE_NAME:-checkfreq_multigpu}"
...
run_model() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/checkfreq_hf_checkpoint_bench.py" \
    --model "${model_path}" \
