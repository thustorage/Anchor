...
# =====================================================================
# Unified e2e CheckFreq checkpoint fault runner.
...
#   ./run_e2e_checkfreq_three_models.sh [single|multi]
# =====================================================================
...
case "${MODE}" in
  single)
    GPU_IDS="${GPU_IDS:-0}"
    REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_checkfreq}"
    CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-${SCRIPT_DIR}/ckpt/e2e_checkfreq}"
    MASTER_PORT_BASE="${MASTER_PORT_BASE:-29831}"
    ;;
  multi)
    GPU_IDS="${GPU_IDS:-0,1}"
    REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_checkfreq_multigpu}"
    CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-${SCRIPT_DIR}/ckpt/e2e_checkfreq_multigpu}"
    MASTER_PORT_BASE="${MASTER_PORT_BASE:-30141}"
    ;;
  *)
...
# NOTE: the checkfreq bench builds its engine via native.create_engine, whose
# ds-config does not consume a sub_group_size override, so --zero3-sub-group-size
# is intentionally NOT passed here (the bench does not accept it).
...
cleanup() {
  ...
  pkill -f "e2e_checkfreq_checkpoint_fault_bench.py --phase worker" 2>/dev/null || true
...
run_model() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/e2e_checkfreq_checkpoint_fault_bench.py" \
    --model "${model_path}" \
