...
# =====================================================================
# Unified e2e DS-native checkpoint fault runner.
...
#   ./run_e2e_ds_native_three_models.sh [single|multi]
# =====================================================================
...
case "${MODE}" in
  single)
    GPU_IDS="${GPU_IDS:-0}"
    REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_ds_native}"
    CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-${SCRIPT_DIR}/ckpt/e2e_ds_native}"
    MASTER_PORT_BASE="${MASTER_PORT_BASE:-29831}"
    ;;
  multi)
    GPU_IDS="${GPU_IDS:-0,1}"
    REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_ds_native_multigpu}"
    CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-${SCRIPT_DIR}/ckpt/e2e_ds_native_multigpu}"
    MASTER_PORT_BASE="${MASTER_PORT_BASE:-30131}"
    ;;
  *)
...
cleanup() {
  ...
  pkill -f "e2e_ds_native_fault_bench.py --phase worker" 2>/dev/null || true
...
run_model() {
  ...
  "${PYTHON_BIN}" "${SCRIPT_DIR}/e2e_ds_native_fault_bench.py" \
    --model "${model_path}" \
