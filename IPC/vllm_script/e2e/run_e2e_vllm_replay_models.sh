...
# =====================================================================
# Unified replay e2e fault runner. Single-GPU is just GPU_IDS with one
# id (tensor_parallel_size derived from the count). Pick ONE mode below.
# Mode A (single-GPU) is active; comment it and uncomment Mode B for TP>1.
# =====================================================================

# --- Mode A: single-GPU (TP=1) --------------------------------------
GPU_IDS="${GPU_IDS:-0}"
REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_vllm_replay}"

# --- Mode B: multi-GPU (TP=visible GPUs) ----------------------------
# GPU_IDS="${GPU_IDS:-0,1}"
# REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_vllm_replay_multigpu}"
...
run_model() {
  ...
  echo "[run] label=${label} model=${model_path} gpu_ids=${GPU_IDS} tp=${TENSOR_PARALLEL_SIZE} total_requests=${TOTAL_REQUESTS} concurrency=${CONCURRENCY} fault_frequency=${FAULT_FREQUENCY} fault_check_interval_seconds=${FAULT_CHECK_INTERVAL_SECONDS}"
  CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PYTHON_BIN}" "${SCRIPT_DIR}/e2e_vllm_replay_fault_bench.py" \
    --model "${model_path}" \
