...
# =====================================================================
# Unified KV-memory e2e fault runner. Single-GPU is just GPU_IDS with
# one id (tensor_parallel_size derived from the count). Pick ONE mode.
# Mode A (single-GPU) is active; comment it and uncomment Mode B for TP>1.
# =====================================================================

# --- Mode A: single-GPU (TP=1) --------------------------------------
GPU_IDS="${GPU_IDS:-0}"
REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_vllm_kv_memory}"
KV_CHECKPOINT_TAG="${KV_CHECKPOINT_TAG:-e2e_vllm_kv_memory}"

# --- Mode B: multi-GPU (TP=visible GPUs) ----------------------------
# GPU_IDS="${GPU_IDS:-0,1}"
# REPORT_ROOT="${REPORT_ROOT:-${SCRIPT_DIR}/report1/e2e_vllm_kv_memory_multigpu}"
# KV_CHECKPOINT_TAG="${KV_CHECKPOINT_TAG:-e2e_vllm_kv_memory_multigpu}"
...
# KV flush/commit cadence K (steps). K=1: cuda-sync + commit every step (original behavior,
# max recovery completeness). K>1: block copies go async on copy_stream (overlap decode) and
# sync+commit every K steps — lower steady-state overhead, recovery re-decodes up to K extra
# steps of tail. Inherited by manager + all workers. Set to 1 to fully revert.
export VLLM_KV_FLUSH_INTERVAL="${KV_FLUSH_INTERVAL:-1}"
...
# Sweep /dev/shm artifacts: the on-shm KV checkpoint file, the CPU KV buffer
# segments, and vLLM CUDA-IPC / torch handles. Named-segment only (never psm_*/sem.*,
# which may belong to unrelated processes). Reports are kept.
sweep_shm() {
  rm -f "${KV_CHECKPOINT_DIR:-/dev/shm}/${KV_CHECKPOINT_TAG}"_* 2>/dev/null || true
...
# Pre-clean: clear stale segments left by a prior crashed/OOM-killed run (whose
# cleanup trap never fired) so this run's engine init does not hit ENOSPC on /dev/shm.
sweep_shm
...
run_model() {
  ...
  local kv_checkpoint_file="${KV_CHECKPOINT_DIR:-/dev/shm}/${KV_CHECKPOINT_TAG}_${label}.pt"
  local -a extra_args=()
  ...
  CUDA_VISIBLE_DEVICES="${GPU_IDS}" "${PYTHON_BIN}" "${SCRIPT_DIR}/e2e_vllm_kv_memory_fault_bench.py" \
    --model "${model_path}" \
    ...
    --kv-checkpoint-file "${kv_checkpoint_file}" \
